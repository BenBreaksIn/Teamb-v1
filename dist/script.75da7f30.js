...
function updateStreamerStatus() {
  streamers.forEach(function (streamer) {
    fetch(streamer.url).then(function (response) {
      return response.text();
    }).then(function (html) {
      var parser = new DOMParser();
      var doc = parser.parseFromString(html, "text/html");
      var streamerOnline = doc.querySelector(".stream-username");
      var streamTitleElement = doc.querySelector(".stream-title");
      if (streamerOnline && streamTitleElement) {
        var streamTitle = streamTitleElement.textContent.trim();
      ...
      console.log("Error fetching data for ".concat(streamer.name, ": ").concat(error));
    });
  });
}
function updateStreamerElement(streamer) {
  var onlineElement = document.getElementById(streamer.elementId);
  var offlineElement = document.getElementById(streamer.offlineElementId);
  if (streamer.status) {
